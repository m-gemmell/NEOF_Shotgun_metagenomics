...
```{bash eval=FALSE}
ln -s /pub39/tea/nsc006/NEOF/Shotgun_metagenomics/raw_fastq/* .
```
...
We use the `-f` parameter option to output reads that only include the SAM flag. In this case `4` stands for unmapped reads. Therefore, our resulting fastq files will only contain unmapped reads. The following link is very useful to create a SAM flag you may need: https://broadinstitute.github.io/picard/explain-flags.html.

This step may make unmatched paired files. This occurs when a read from R2 is removed but the matching read in R1 is not removed, or vice versa. This will cause issues for further analysis. 
...
mkdir K1_fullset
cd K1_fullset
ln -s /pub39/tea/matthew/NEOF/Shotgun_metagenomics/binning/K1_fullset/* .
```
...
```{bash eval=FALSE}
#Copy over directory
cp -r /pub39/tea/matthew/NEOF/Shotgun_metagenomics/prokka/K1_fullset ~/8-metawrap_anno
...
<!--chapter:end:12-Assembly_functional_annotation.Rmd-->
...
The below diagram describes the `HUMAnN` pipeline.

```{r, fig.align = 'center',out.width= '90%', echo=FALSE }
knitr::include_graphics(path = "figures/humann_workflow.png", auto_pdf = TRUE)
``` 
...
## HUMAnN: Optional task

Carry out all the steps starting from [Multi sample processing](#humannmultisampleprocessing) with the gene.

- Copy the data from `/pub39/tea/nsc006/NEOF/Shotgun_metagenomics/DietGeneFamilies`
- Ensure in the [renormalising data](#humannrenormalise) step you set the `--units` option to `cpm`.
- On top of analysing the unstratified data you can also analyse the stratified date.
