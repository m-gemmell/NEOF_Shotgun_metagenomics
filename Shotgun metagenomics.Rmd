...
```{bash eval=FALSE}
cp /pub39/tea/nsc006/NEOF/Shotgun_metagenomics/GRCh38_slice.fasta .
```
...
- __`out2=`__: The output R2 fastq file of unmapped unpaired reads.
...
```
3. What percentage of the __root__ was classified to __Proteobacteria__ for __W1__? `r longmcq(opts_p)`
...
Viewing the Bracken output files with your favourite text viewer (`less`, `nano`, `vim`, etc.), attempt the below MCQs.
...
List the contents of the output directory and you'll see there are 2 fasta files with the prefix of `K1`. These are the 2 bins that will hopefully contain 1 MAG (Metagenome-Assembled Genome).
...
- `-x` : Suffix/extension of bin files. Other files are ignored in the specified bin directory.
   - `fa` is used as our `MetaBAT2` produce fasta files that end in `.fa`.
...
First step is to extract the completeness and contamination fields/columns.
...
The __final__ step is to carry out the overall quality calculation and append the information to the "MAGS_quality.csv" file.
...
The last step is to combine the files "MAGS_checkm.csv" and "MAGS_quality.csv" with the `paste` command. The `-d ","` options indicates the merged files will be separated by commas (`,`), matching the column separation in "MAGS_checkm.csv".
...
```
5. What is the completeness value of K1.27? `r longmcq(opts_p)`

```{r, echo = FALSE}
opts_p <- c("__4.17__", "__33.62__", answer="__172__")
```
6. How many genomes are within the K1.12 bin? `r longmcq(opts_p)`
...
That is quite a bit of code. You don't need to undertsand it as it should always work for `PROKKA` output. 
...
Next we loop through each file prefix and use the variable `bin` (arbitrarily chosen) which contains the file prefix. 
...
A good way to figure out what a pipe workflow is by building it up step by step.
...
#/g indicates to carry it out for every instance in every line
#Default is to only substitute the first instance in each line
cat K1.1.gff | grep "eC_number=" | cut -f 9 | cut -f 1,2 -d ';' | \
sed "s/ID=//g" | head
